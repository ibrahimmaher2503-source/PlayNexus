...
@section('title', __('staff.invite_title') . ' · PlayNexus')
...
@section('content')
    <main class="mx-auto min-h-screen max-w-3xl px-4 py-6 sm:px-6">
        <header class="border-b border-[var(--pn-border)] pb-5">
            ...
            <h1 class="mt-1 text-2xl font-bold">{{ __('staff.invite_title') }}</h1>
            <p class="mt-1 max-w-2xl text-sm text-[var(--pn-ink-muted)]">{{ __('staff.invite_description') }}</p>
        </header>
        ...
        <section class="mt-8 max-w-xl rounded-[14px] border border-[var(--pn-border)] bg-[var(--pn-surface)] p-5 shadow-sm sm:p-6" aria-labelledby="staff-invite-form-heading">
            <h2 class="text-lg font-bold" id="staff-invite-form-heading">{{ __('staff.invite_form_caption') }}</h2>
            <p class="mt-1 text-sm text-[var(--pn-ink-muted)]">{{ __('staff.invite_note') }}</p>

            <form class="mt-6 space-y-5" method="POST" action="{{ route('staff.store') }}" aria-describedby="{{ $errors->any() ? 'staff-invite-errors' : 'staff-invite-form-heading' }}">
                @csrf
                <div>
                    <label class="block text-sm font-semibold" for="staff-name">{{ __('staff.invite_name') }}</label>
                ...
                <div>
                    <label class="block text-sm font-semibold" for="staff-email">{{ __('staff.invite_email') }}</label>
                ...
                <button class="inline-flex min-h-11 items-center rounded-[10px] bg-[var(--pn-primary)] px-4 font-semibold text-[var(--pn-surface)] hover:bg-[var(--pn-primary-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--pn-focus)] focus:ring-offset-2" type="submit">{{ __('staff.invite_submit') }}</button>
            </form>
        </section>
    </main>
@endsection
